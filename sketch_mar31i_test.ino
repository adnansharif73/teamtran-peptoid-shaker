...
void startExperiment() {
  ...
  // Update the display with the initial experiment settings
  updateDisplay(); // Display the initial settings
  
  // Display the experiment progress
  updateExperimentDisplay(0, currentCycle);

  // Move the motor with the desired settings
  for (int cycle = 0; cycle < numCycles && experimentRunning; cycle++) {
    currentCycle = cycle; // Update current cycle
    
    // Update the display with the experiment progress
    updateExperimentDisplay((millis() - experimentStartTime) / 1000, currentCycle);

    // Rotate the motor
    rotateMotor(FORWARD, HA_speed);

    // Check for stop button press while the motor is moving
    while (digitalRead(BUTTON_OFF_PIN) == HIGH) {
      // Continue rotating the motor
      rotateMotor(FORWARD, HA_speed);
      delay(pauseTime * 1000); // Delay between steps
    }

    // Check if experiment has been stopped
    if (!experimentRunning) break; // Check if experiment is stopped
    ...
    // Check for stop button press while the motor is moving in reverse
    while (digitalRead(BUTTON_OFF_PIN) == HIGH) {
      // Continue rotating the motor
      rotateMotor(REVERSE, LA_speed);
      delay(pauseTime * 1000); // Delay between steps
  ...
  // Experiment is finished, continuously update the display with the final state
  while (experimentRunning) {
    updateDisplay(); // Constantly update the display with the final state
    updateExperimentDisplay((millis() - experimentStartTime) / 1000, currentCycle); // Update experiment progress
  }
}
...
void stopExperiment() {
  ...
  while (!digitalRead(BUTTON_OFF_PIN)) {
    delay(100); // Wait for the button to be released
  }
}
...
void updateExperimentDisplay(unsigned long startTime, int currentCycle) {
